...
function ChatsList() {
  ...
  const highlightQuery = (text) => {
    ...
    return parts.map((part, i) => (
      <span key={i} className={part.toLowerCase() === searchQuery.toLowerCase() ? "bg-yellow-500 text-black" : ""}>
        {part}
      </span>
    ));
  };
...
// import NoChatsFound from "./NoChatsFound";
// import { IoMdArchive, IoMdTrash } from "react-icons/io";
// import { FaImage, FaVideo, FaFileAudio, FaFile, FaPinterest } from "react-icons/fa";
// import toast from "react-hot-toast";
// import { IoArchiveOutline, IoPinSharp, IoTrash } from "react-icons/io5";
// import { FiMapPin } from "react-icons/fi";
// import { BanIcon, Blocks, PinIcon } from "lucide-react";
...
//     getFilteredChats,
//     setSearchResults,
//     searchResults,  // New: Use searchResults for deep search
//     isUsersLoading,
//     setSelectedUser,
//     archiveChat,
//     selectedUser,
...
//     getMessagesByGroupId,
//     getMessagesByUserId,
//     setTargetHighlightMsgId,  // New: To set highlight on click
//   } = useChatStore();
//   const { onlineUsers, theme,authUser, socket } = useAuthStore();
//   const [active, setActive] = useState(null);
...
//     getMyChatPartners(),
//     getGroupChats();
//     setSearchResults([]); // initial data already loaded globally
//   }
// }, [searchQuery, getFilteredChats, setSearchResults]);
//   const chatList = useMemo(() => {
//     let list = searchQuery ? searchResults : [...chats, ...groupChats, ...archivedChats];
//     list.sort((a, b) => new Date(b.recentMessage?.createdAt || 0) - new Date(a.recentMessage?.createdAt || 0));
//     return list;
...
//     } else {
//       let prefix= "You";
//       prefix = message?.deletedBy?.includes(authUser._id) ? <BanIcon size={13}/> : message.isForwarded ? "Forwarded: " : message.isEdited ? "Edited: " : ((message.senderId === authUser._id || message.senderId?._id === authUser._id) && (chat._id !== authUser._id)) ? "You: " : (chat.name ? `${chat.members?.find((mem) => (mem._id === message.senderId || mem._id === message.senderId?._id))?.fullName?.split(" ")[0]?.replace(chat.members?.find((mem) => mem._id === message.senderId)?.fullName?.split("")[0], chat.members?.find((mem) => mem._id === message.senderId)?.fullName?.split("")[0].toUpperCase())}: ` : "user: ");
//       if (message.text && message.text.trim() !== "") {
//         return <>{prefix === "undefined:" ? "You:" : prefix || "You:"}{highlightQuery(truncatePar(message.text, 15))}</>;
//      // return prefix + highlightQuery(truncatePar(message.text, 22));
// //       }
//       }
//       if (message.originalName) {
//         const name = truncatePar(message.originalName, 22);
...
// }, [socket])
//   const handleChatClick = (chat) => {
//    //   console.log(chat.recentMessage)
//    if (searchQuery && chat.recentMessage) {
//      setTargetHighlightMsgId(chat.recentMessage.text);
//  } 
...
//     }
//     // New: If from content search, set highlight target
...
//   if (isUsersLoading && chatList.length === 0) return <UsersLoadingSkeleton />;
//   if (chatList.length === 0) return <NoChatsFound />;
...
//           px-2 py-4 mb-2 rounded-lg overflow-hidden cursor-pointer hover:bg-cyan-500/20 transition-colors `}
//         >
//           <div className={`flex items-center  w-full gap-3 ${active === chat._id ? "pointer-events-none" : ""} `}>
//             <div className="relative">
//               <img
...
//               />
//               {!chat.name && chat.onlineStatus === "online" && (
//                 <span className="absolute top-0 left-0 w-3 h-3 bg-cyan-500 rounded-full border-2 border-slate-900"></span>
//               )}
//             </div>
//             <div className="flex-1 w-full">
//               <div className="flex w-full items-center justify-between">
//                 <h3 style={{color: chat.color}} className="text-sm font-semibold capitalize text-slate-200  text-nowrap text-[0.8rem] ">{truncatePar(chat.name || chat.fullName, 15)}</h3>
//                 {chat.recentMessage && (
//                   <span className="text-xs text-[0.65rem] text-nowrap text-slate-400">
//                     {new Date(chat.recentMessage.createdAt).toLocaleTimeString([], {
//                       hour: "2-digit",
//                       minute: "2-digit",
//                     })}
...
//               </div>
//               <div className="flex items-center justify-between">
//                 <p className={`text-xs ${theme !== "light" ? "text-blue-100" : "text-slate-800"} flex items-center `}>{getDisplay(chat.recentMessage, chat)}</p>
//                 {chat.unreadCount > 0 && (
//                   <span className="bg-cyan-600 text-[var(--text-primary)] text-xs rounded-full px-2 py-1">{chat.unreadCount}</span>
//                 )}
//               </div>
//             </div>
//           </div>
//           {active === chat._id  && (
//             <div className={`absolute right-2 top-2 flex flex-col bg-slate-800 ${theme === "light" ? "bg-slate-100" : "bg-slate-800"}  rounded-lg shadow-lg z-30 min-w-[10rem] p-2 "`}>
//               <button
//                 onClick={() => handleArchive(chat._id, !!chat.name)}
//                 className={`flex items-center gap-2 px-4 py-2 text-sm text-slate-200 ${theme !== "light" ? "text-slate-200" : "text-slate-800"} hover:bg-cyan-500/20`}
//               >
//                 <IoArchiveOutline size={19} /> Archive
//               </button>
//               <button
